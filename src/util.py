...
def get_quilt_versions(mc_version: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if not is_minecraft_version_supported(mc_version):
            logging.warning(
                f"Minecraft версия {mc_version} не поддерживается Quilt",
            )
            return []
        ...
        for loader in loader_versions:
            ...
            if hasattr(loader, "minecraft_version") and mc_version in str(
        ...
        logging.exception("minecraft-launcher-lib не установлен")
        return []
    except Exception as e:
...
def save_ely_session(settings, session_data):
    ...
    logging.info(
        f'Сессия Ely.by сохранена: username={session_data.get("username")}, uuid={session_data.get("uuid")}',
    )
...
def load_ely_session(settings):
    ...
    if settings.get("ely_logged_in", False):
        ...
        logging.info(
            f'Загружена сессия Ely.by: username={session["username"]}, uuid={session["uuid"]}',
        )
        return session
