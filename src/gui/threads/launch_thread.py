...
class LaunchThread(QThread):
    ...
    def run(self):
        try:
            self.log(
                f"Starting Minecraft launch process for user: {self.username}",
            )
                ...
                options.update(
                    {
                        ...
                        + [
                            f"-javaagent:{AUTHLIB_JAR_PATH}=https://authserver.ely.by",
                        ],
                    },
                )
            ...
            if self.loader_type == "forge":
                ...
                if not forge_version:
                    raise Exception(
                        f"Forge version for {self.version_id} not found",
                    )
                launch_version = (
                ...
                try:
                    ...
                    launch_version = (
                        f"fabric-loader-{loader_version}-{self.version_id}"
                    )
                ...
                try:
                        ...
                        for name in os.listdir(versions_dir):
                            try:
                                if not isinstance(name, str):
                                    continue
                                if (
                                    self.version_id in name
                                    and "OptiFine" in name
                                ):
                    ...
                    if not candidates:
                        raise Exception(
                            "OptiFine profile not found; please install OptiFine for this version",
                        )
    ...
    def apply_legacy_patch(self, version: str):
        ...
        try:
            jar_path = os.path.join(
                MINECRAFT_DIR, "versions", version, f"{version}.jar",
            )
    ...
    def _set_status(self, text):
        self.progress_update_signal.emit(
            self.current_step, self.total_steps, text,
        )
