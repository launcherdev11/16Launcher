...
class SettingsTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.memory_value_label.setStyleSheet(
        ...
        self.show_console_checkbox.toggled.connect(
            self.hide_console_checkbox.setVisible,
        )
        ...
        self.hide_console_checkbox.setVisible(
            self.show_console_checkbox.isChecked(),
        )

        self.close_on_launch_checkbox = QCheckBox(
        ...
        self.check_running_processes_checkbox = QCheckBox(
            "Проверять запущенные процессы Minecraft",
        )
        ...
        self.choose_mods_directory_button.clicked.connect(
            self.choose_mods_directory,
        )
        ...
        self.auto_java_checkbox_versions = QCheckBox(
            "Автоматическая установка Java",
        )
        ...
        if self.parent_window is not None:
            self.ely_logout_button.clicked.connect(
                self.parent_window.ely_logout,
            )
        ...
        self.auto_update_checkbox.setStyleSheet(
            self.check_updates_checkbox.styleSheet(),
        )
        ...
        if self.parent_window and hasattr(
            ...
            self.check_updates_now_btn.clicked.connect(
                lambda: self.parent_window.check_for_updates(auto=False),
            )
        ...
        def save_updates_settings():
            if self.parent_window:
                self.parent_window.settings["check_updates_on_start"] = (
                    self.check_updates_checkbox.isChecked()
                )
                self.parent_window.settings["auto_update"] = (
        ...
        if "close_on_launch" in settings:
            self.close_on_launch_checkbox.setChecked(
        ...
        if "memory" in settings:
            ...
            value = max(
                ...
                min(self.memory_slider.maximum(), value),
            )
        ...
        if "hide_console_after_launch" in settings:
            self.hide_console_checkbox.setChecked(
                settings["hide_console_after_launch"],
            )
        if "check_running_processes" in settings:
            self.check_running_processes_checkbox.setChecked(
                settings["check_running_processes"],
            )
        ...
        self.auto_java_checkbox_game.toggled.connect(
            self._on_auto_java_toggled_from_game,
        )
        self.auto_java_checkbox_versions.toggled.connect(
            self._on_auto_java_toggled_from_versions,
        )
        ...
        if "check_updates_on_start" in settings:
            self.check_updates_checkbox.setChecked(
                bool(settings["check_updates_on_start"]),
            )
    ...
    def choose_directory(self):
        try:
            ...
            if directory:
                ...
                # Сохраняем в настройках
                if self.parent_window:
                    self.parent_window.settings["minecraft_directory"] = (
                        directory
                    )
                    save_settings(self.parent_window.settings)
    ...
    def save_console_settings(self):
        ...
        if self.parent_window:
            self.parent_window.settings["show_console"] = (
                self.show_console_checkbox.isChecked()
            )
            self.parent_window.settings["hide_console_after_launch"] = (
                self.hide_console_checkbox.isChecked()
            )
            save_settings(self.parent_window.settings)
