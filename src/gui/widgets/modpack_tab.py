...
from PyQt5.QtCore import QEvent, QObject, QRegExp, QSize, Qt
...
class ModpackTab(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.BANNER_THUMB_H = int(
            self.BANNER_H * (self.BANNER_THUMB_W / self.BANNER_W),
        )
        ...
        try:
            ...
            for mod_item in mods_list:
                if isinstance(mod_item, dict):
                    mod_name = (
                        mod_item.get("file") or mod_item.get("name") or ""
                    )
                    if mod_name:
                        count += 1
                elif mod_item:
    ...
    def open_modpack_details(self, pack_data: dict[str, Any]) -> None:
        ...
        banner.setStyleSheet(
            "background-color: #3A3A3A; border: 1px solid #555555; border-radius: 12px;",
        )
        ...
        subtitle.setStyleSheet(
            "color:#BBBBBB; font-size:18px; margin-left:8px;",
        )
        ...
        def pill(text: str) -> QLabel:
            l = QLabel(text)
            l.setStyleSheet(
                "background-color:#404040; border: 1px solid #555555; border-radius:12px; padding:6px 10px; font-size:13px;",
            )
            return l
        ...
        for b in (btn_desc, btn_det):
            b.setCheckable(True)
            b.setStyleSheet(
                "QPushButton{background-color:#404040; border:1px solid #555555; border-radius:14px; padding:8px 16px;} QPushButton:checked{background-color:#505050;}",
            )
        ...
        desc_box.setStyleSheet(
            "background-color:#404040; border:1px solid #555555; border-radius:12px; padding:14px; font-size:14px;",
        )
        ...
        action_btn.clicked.connect(
            lambda: (self.launch_modpack(pack_data), dialog.accept()),
        )
    ...
    def get_modpack_size(self, pack_data: dict[str, Any]) -> str:
        try:
            ...
            estimated_size = (
                (mods_count * 5) + (textures_count * 10) + (shaders_count * 15)
            )
    ...
    def show_context_menu(self, pack_data: dict[str, Any]) -> None:
        ...
        duplicate_action.triggered.connect(
            lambda: self.duplicate_modpack(pack_data),
        )
    ...
    def edit_modpack(self, pack_data: dict[str, Any]) -> None:
        ...
        self.icon_preview_edit.setStyleSheet(
            "background-color: #3A3A3A; border: 1px solid #555555; border-radius: 8px;",
        )
        ...
        if current_icon_name:
            ...
            if os.path.exists(current_icon_path):
                self.icon_preview_edit.setPixmap(
                    QPixmap(current_icon_path).scaled(
                        self.ICON_W,
                        self.ICON_H,
                        Qt.KeepAspectRatioByExpanding,
                        Qt.SmoothTransformation,
        ...
        self.add_mod_btn.clicked.connect(
            lambda: self.add_mods_to_pack(pack_data),
        )
            ...
            for file_path in selected_files:
                ...
                if not os.path.exists(dest_path):
                    try:
                        ...
                        logging.exception(
                            f"Ошибка копирования мода {mod_name}: {e}",
                        )
                        continue
                ...
                for mod_item in mods_list:
                    ...
                    if mod_name:
                        try:
                            ...
                            logging.warning(
                                f"Мод {mod_name} отсутствует в архиве",
                            )

                with open(
                    os.path.join(
                        self.modpacks_dir,
                        f'{pack_data["name"]}.json',
                    ),
                    "w",
                ) as f:
    ...
    def show_creation_dialog(self):
        ...
        self.pack_name.setValidator(
            QRegExpValidator(QRegExp("^[A-Za-z0-9()]{0,32}$"), self),
        )
        ...
        self.pack_description.setSizePolicy(
        ...
        self.banner_preview.setFixedSize(
            self.BANNER_THUMB_W,
            self.BANNER_THUMB_H,
        )
        self.banner_preview.setStyleSheet(
            "background-color: #3A3A3A; border: 1px solid #555555; border-radius: 12px;",
        )
        ...
        def update_lists():
            ...
            if self.use_textures_cb.isChecked():
                ...
                self.textures_selection.addItems(
                    ModManager.get_textures_list(version),
                )
            else:
                self.textures_label.setVisible(False)
                self.textures_selection.setVisible(False)
            ...
            if self.use_shaders_cb.isChecked():
                ...
                self.shaders_selection.addItems(
                    ModManager.get_shaders_list(version),
                )
            else:
                self.shaders_label.setVisible(False)
                self.shaders_selection.setVisible(False)
        ...
        def move_to_trash(path: str) -> str:
            ...
            try:
                ...
                logging.exception(
                    f"Не удалось переместить в корзину {path}: {e}",
                )
                return ""
            ...
            if action.get("type") == "delete":
                ...
                for dest, _src in action["ops"]:
                    try:
                        ...
                        logging.exception(
                            f"Ошибка отката добавления {dest}: {e}",
                        )
                self._last_action_step2 = None

        def setup_dnd(
            list_widget: QListWidget,
            target_dir_getter: Callable[[], str],
        ...
        def on_mods_add() -> None:
            ...
            ops = add_files_to(
                os.path.join(
                    ModManager.get_mods_directory(),
                    self.pack_version.currentText(),
                ),
                files,
                self.mods_selection,
            )
        ...
        mods_del_btn.clicked.connect(
            lambda: handle_delete(
                self.mods_selection,
                os.path.join(
                    ModManager.get_mods_directory(),
                    self.pack_version.currentText(),
        ...
        textures_del_btn.clicked.connect(
            lambda: handle_delete(self.textures_selection, RESOURCEPACKS_DIR),
        )
        ...
        shaders_del_btn.clicked.connect(
            lambda: handle_delete(self.shaders_selection, SHADERPACKS_DIR),
        )
    ...
    def select_image(self, kind: str = "icon"):
        ...
        if kind == "icon":
            ...
            if (
                hasattr(self, "icon_preview_edit")
                and self.icon_preview_edit is not None
            ):
            ...
            if (
                hasattr(self, "banner_preview")
                and self.banner_preview is not None
            ):
    ...
    def save_modpack(self, dialog):
        if not self.pack_name.hasAcceptableInput() or not self.pack_name.text():
            QMessageBox.warning(
                self,
                "Неверное имя",
                "Имя может содержать латинские буквы (A-Z, a-z), цифры и скобки () и быть не длиннее 32 символов.",
            )
            return
        ...
        selected_mods = [
            item.text() for item in self.mods_selection.selectedItems()
        ]
        selected_textures = [
            item.text() for item in self.textures_selection.selectedItems()
        ]
        selected_shaders = [
            item.text() for item in self.shaders_selection.selectedItems()
        ]
    ...
    def use_current_parameters(self):
        ...
        try:
            ...
            if current_loader_data in ["forge", "fabric"]:
                ...
                loader_text = (
                    "Forge" if current_loader_data == "forge" else "Fabric"
                )
            ...
            logging.exception(
                f"Ошибка при использовании текущих параметров: {e}",
            )
            QMessageBox.critical(
                self,
                "Ошибка",
                f"Не удалось применить текущие параметры: {e}",
            )
    ...
    def check_java_version(self):
        ...
        try:
            ...
            if version_match:
                ...
                min_java, description = requirements.get(
                    loader,
                    (8, "Требования Java зависят от модлоадера"),
                )
