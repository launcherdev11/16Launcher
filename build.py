...
def check_dependencies():
    ...
    try:
        ...
        print_error(
            "PyInstaller не установлен! Установите: pip install pyinstaller",
        )
        return False
    ...
    if iscc_exe:
        try:
            ...
            if os.path.exists("installer_output/16Launcher_Setup.exe"):
                print_success(
                    "Установщик Windows создан: installer_output/16Launcher_Setup.exe",
                )
                return True
        ...
        print_warning(
            "Inno Setup не найден. Пропускаем создание установщика Windows",
        )
        print_warning(
...
def main():
    ...
    if args.platform in ["windows", "all"]:
        if not build_windows():
            success = False
        if args.installer and not create_windows_installer():
            success = False

    if args.platform in ["linux", "all"]:
        if not build_linux():
            success = False
        if args.installer and not create_linux_installer():
            success = False

    if args.platform in ["macos", "all"]:
        if not build_macos():
            success = False
        if args.installer and not create_macos_installer():
            success = False
